...
function App() {
  ...
  const claimNFTs = () => {
    let cost = CONFIG2.WEI_COST;
    let gasLimit = CONFIG2.GAS_LIMIT;
    let totalCostWei = String(cost * mintAmount);
    let totalGasLimit = String(gasLimit * mintAmount);
    console.log("Cost: ", totalCostWei);
    console.log("Gas limit: ", totalGasLimit);
    setFeedback(`Minting your ${CONFIG2.NFT_NAME}...`);
    setClaimingNft(true);
    blockchain.smartContract.methods
    .mintNFTs(mintAmount) //mintAmount, "https://ipfs.io/ipfs/Qmf5ArUFQyuYBd591Ext3Z4WzXjWzxAd8vciF6dfbmC1P6?filename=gh.gif"
    .send({
      ...
      .then((receipt) => {
        console.log(receipt);
        setFeedback(
          `Congratulations, the ${CONFIG2.NFT_NAME} is yours! go visit Opensea.io to view it.`
        );
        setClaimingNft(false);
        dispatch(fetchData(blockchain.account));
      });
      ...
      > 
        ...
        <ResponsiveWrapper flex={1} style={{ padding: 24 }} test>
          ...
          >
            ...
            >
              {data.totalSupply} / {CONFIG2.MAX_SUPPLY}
            </s.TextTitle>
            <s.TextDescription
              style={{
                fontFamily: 'serif',
            ...
            >
              <StyledLink target={"_blank"} href={CONFIG2.SCAN_LINK}>
                {truncate(CONFIG2.CONTRACT_ADDRESS, 15)}
              </StyledLink>
            </s.TextDescription>
            {/* <span
              style={{
            ...
            <s.SpacerSmall />
            {Number(data.totalSupply) >= CONFIG2.MAX_SUPPLY ? (
              <>
                <s.TextTitle
                  style={{ textAlign: "center", color: "var(--accent-text)",fontFamily: 'serif', }}
                ...
                <s.TextDescription
                  style={{ textAlign: "center", color: "var(--accent-text)",fontFamily: 'serif', }}
                >
                  You can still find {CONFIG2.NFT_NAME} on
                </s.TextDescription>
                <s.SpacerSmall />
                <StyledLink target={"_blank"} href={CONFIG2.MARKETPLACE_LINK}>
                  {CONFIG2.MARKETPLACE}
                </StyledLink>
              </>
            ) : (
              <>
                <s.TextTitle
                  style={{ textAlign: "center", color: "var(--accent-text)",fontFamily: 'serif', }}
                >
                  1 {CONFIG2.SYMBOL} costs {CONFIG2.DISPLAY_COST}{" "}
                  {CONFIG2.NETWORK.SYMBOL}
                </s.TextTitle>
                <s.SpacerXSmall />
                <s.TextDescription
                  style={{ textAlign: "center", color: "var(--accent-text)", fontFamily: 'serif', }}
                ...
                blockchain.smartContract === null ? (
                  ...
                  <>
                    ...
                    <s.Container ai={"center"} jc={"center"} fd={"row"}>
                      ...
                      >
                        {claimingNft ? "mint DAO" : "minting DAO"}
                      </StyledButton> 
                    </s.Container> 
                  </>
                )}
